...
1. Read `00-case.md`, `10-people.md`, and `20-route-options.md`. State any missing fact that changes eligibility; do not infer it from nationality, name, or travel history.
...
5. Add each claim to `30-requirements.md` and its source to the source register there. Use one requirement per evidence obligation, not one requirement per webpage.

## Source record

For every `SRC-*`, write: official URL, publisher, retrieved date, any page update date, short paraphrased rule, applicable route/person, and `Fresh until`. Follow [source-reconciliation.md](references/source-reconciliation.md) when sources disagree. Mark a conflict rather than selecting a convenient answer. Request legal review if authoritative sources cannot reconcile it.
