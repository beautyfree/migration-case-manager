...
3. Read `references/case-format.md` before editing a case. Preserve IDs and file ownership.
4. Start with `00-case.md` and `10-profile.md`; then delegate rule discovery to `$migration-research` and executable work to `$migration-actions`.

## Operate the case

Treat `20-requirements.md` as a graph, not a generic checklist. Every requirement must have a source ID, a specific affected person, evidence IDs, and actions. Keep the route, departure logistics, and home-country affairs as separate requirement groups.
