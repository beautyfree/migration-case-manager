...
| 2026-07-30 | Advanced `R8-04`: TypeScript validation now matches the deterministic Python safety gates for source freshness/conflicts, document chains and quality, action receipts/decisions, appointments, and readiness age. Native negative-fixture tests and contract checks pass. `migration-os migrate` now creates a separately validated v2 case without changing its v1 source; deterministic reports and work-queue generators remain to be ported. |
