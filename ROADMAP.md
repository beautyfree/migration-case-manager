...
| 2026-07-31 | Advanced `R8-07`: added the dependency-free `migration-os` npm launcher. It maps Node platform/architecture (including glibc versus musl) to the existing native release artifact, rejects mutable/non-HTTPS releases, previews exact checksum before consent, verifies the downloaded binary, runs `doctor` before activation, and forwards native CLI arguments. GitHub Actions run `30590379518` passed Node launcher tests, `npm pack --dry-run`, existing bootstrap safety checks, complete artifact build, and the macOS/Linux/Windows standalone matrix. GitHub/NPM release publication was still pending at this point. |
| 2026-07-31 | Published preview `v0.1.0-preview.2` and `migration-os@0.1.0-preview.2`. GitHub Actions run `30592796983` passed the eight-artifact build, macOS/Linux/Windows native smoke matrix, immutable GitHub prerelease creation, and npm Trusted Publishing through GitHub OIDC. The initial `preview.1` npm publish bootstrapped the registry package and trust relationship; no npm automation token is stored. |
