...
| 2026-07-31 | Advanced `R8-07`: added the dependency-free `@beautyfree/migration-os` npm launcher. It maps Node platform/architecture (including glibc versus musl) to the existing native release artifact, rejects mutable/non-HTTPS releases, previews exact checksum before consent, verifies the downloaded binary, runs `doctor` before activation, and forwards native CLI arguments. GitHub Actions run `30590379518` passed Node launcher tests, `npm pack --dry-run`, existing bootstrap safety checks, complete artifact build, and the macOS/Linux/Windows standalone matrix. GitHub/NPM release publication remains deliberately pending. |
